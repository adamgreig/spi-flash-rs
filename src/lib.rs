...
/// Async trait for objects which provide access to SPI flash.
///
/// Provides the same methods as [FlashAccess] but async.
///
/// `From<FlashAccess::Error>` must be implemented for `spi_flash::Error`; for example in your
/// implementation code, add:
///
...
pub trait AsyncFlashAccess {
    ...
    ///
    /// Returns the received data.
    async fn exchange(&mut self, data: &[u8]) -> core::result::Result<Vec<u8>, Self::Error>;
...
impl<'a, A: AsyncFlashAccess> AsyncFlash<'a, A>
where
    Error: From<<A as AsyncFlashAccess>::Error>,
{
    #[cfg(feature = "std")]
    const DATA_PROGRESS_TPL: &'static str =
        " {msg} [{bar:40.cyan/black}] {bytes}/{total_bytes} ({bytes_per_sec}; {eta_precise})";
    ...
    /// `self.id` is updated with the new ID; use `get_id()` to
    /// retrieve it without re-reading the ID from the flash.
    pub async fn read_id(&mut self) -> Result<FlashID> {
        log::debug!("Reading SPI Flash ID");

        let legacy_id = self.release_power_down().await?;
        self.reset().await?;

        let (bank_long, mfn_id_long, device_id_long) = self.read_jedec_id().await?;
        let (bank_short, mfn_id_short, mut device_id_short) = self.read_device_id().await?;
        let unique_id = self.read_unique_id().await?;
    ...
    /// and erase sector opcode. Additionally, larger erase commands
    /// described by the SFDP parameters will be used when appropriate.
    pub async fn read_params(&mut self) -> Result<Option<FlashParams>> {
        log::debug!("Reading SFDP data");

        // Read just SFDP header to get NPH first.
        let data = self.read_sfdp(0, 8).await?;
        let nph = data[6] as usize + 1;
        ...
        // After this parse is successful, however, subsequent errors
        // are returned as errors.
        let data = self.read_sfdp(0, 8 + nph * 8).await?;
        let header = match SFDPHeader::from_bytes(&data) {
            Ok(header) => header,
            Err(_) => return Ok(None),
        };
        ...
        // Read SFDP table data and parse into a FlashParams struct.
        let data = self.read_sfdp(params.ptp, params.plen * 4).await?;
        let params = FlashParams::from_bytes(params.major, params.minor, &data)?;
        self.params = Some(params);
    ...
    pub async fn read_cb<F: Fn(usize)>(
        ...
        for addr in (start..end).step_by(chunk_size) {
            ...
            let mut param = self.make_address(addr as u32);
            param.push(0);
            data.append(&mut self.exchange(Command::FastRead, &param, size).await?);
            cb(data.len());
        }
        cb(data.len());
        Ok(data)
    ...
    /// up to 128 separate SPI exchanges to allow progress to be reported.
    #[cfg(feature = "std")]
    pub async fn read_progress(&mut self, address: u32, length: usize) -> Result<Vec<u8>> {
        let pb = ProgressBar::new(length as u64).with_style(
            ProgressStyle::with_template(Self::DATA_PROGRESS_TPL)
                .unwrap()
                .progress_chars(Self::DATA_PROGRESS_CHARS),
    ...
    pub async fn erase(&mut self) -> Result<()> {
        ...
        self.command(Command::ChipErase).await?;
    ...
    /// otherwise the progress bar is drawn as a spinner.
    #[cfg(feature = "std")]
    pub async fn erase_progress(&mut self) -> Result<()> {
        let time = self.params.map(|p| p.timing.map(|t| t.chip_erase_time_typ));
        let pb = if let Some(Some(time)) = time {
            ProgressBar::new(time.as_millis() as u64).with_style(
                ProgressStyle::with_template(" {msg} [{bar:40.cyan/black}] {elapsed} < {eta}")
    ...
    /// The programming page size is set by `page_size`, which is
    /// automatically set when SFDP parameters are read.
    pub async fn program(&mut self, address: u32, data: &[u8], verify: bool) -> Result<()> {
        self.check_address_length(address, data.len())?;
        ...
        // Read data which will be inadvertently erased so we can restore it.
        let full_data = self.make_restore_data(address, data, &erase_plan).await?;

        // Execute erasure plan.
        self.run_erase_plan(&erase_plan, |_| {}).await?;

        // Write new data.
        let start_addr = erase_plan.0[0].2;
        self.program_data(start_addr, &full_data).await?;
    ...
    pub async fn program_progress(
        ...
        // Execute erasure plan.
        self.run_erase_plan_progress(&erase_plan).await?;

        // Write new data.
        let start_addr = erase_plan.0[0].2;
        self.program_data_progress(start_addr, &full_data).await?;
    ...
    pub async fn reset(&mut self) -> Result<()> {
        ...
        if do_66_99 {
            self.command(Command::EnableReset).await?;
            self.command(Command::Reset).await
        } else if do_f0 {
            self.command(0xF0).await
        } else {
            log::error!("No reset instruction available.");
            Err(Error::NoResetInstruction)
        }
    ...
    /// Check if any block protect bits are set in status register 1.
    pub async fn is_protected(&mut self) -> Result<bool> {
        log::debug!("Checking if BP bits are set");
        let status1 = self.read_status1().await?;
        let (bp0, bp1, bp2) = status1.get_block_protect();
        log::debug!("BP0: {}, BP1: {}, BP2: {}", bp0, bp1, bp2);
        Ok(bp0 || bp1 || bp2)
    }
    ...
    /// the SFDP parameters are used to determine the correct
    /// non-volatile instruction.
    pub async fn protect(&mut self, bp0: bool, bp1: bool, bp2: bool) -> Result<()> {
        log::debug!(
            "Setting block protection bits to BP0={}, BP1={}, BP2={}",
            bp0,
            bp1,
        ...
        let mut status1 = self.read_status1().await?;
        status1.set_block_protect(bp0, bp1, bp2);
        self.write_status1(status1).await?;
        self.wait_while_busy().await?;
        Ok(())
    }
    ...
    /// using the non-volatile commands if supported. If available, the SFDP
    /// parameters are used to determine the correct non-volatile instruction.
    pub async fn unprotect(&mut self) -> Result<()> {
        log::debug!("Checking if BP bits are set before clearing them");
        let mut status1 = self.read_status1().await?;
        let (bp0, bp1, bp2) = status1.get_block_protect();
        if bp0 || bp1 || bp2 {
    ...
    /// flash chip supports the WPS feature or even has a status register 3,
    /// so this command is not called automatically.
    pub async fn unprotect_wps(&mut self) -> Result<()> {
        let mut status3 = self.read_status3().await?;
        if status3.get_wps() {
            log::debug!("WPS bit set, clearing.");
            status3.set_wps(false);
            self.write_status3(status3).await?;
            self.wait_while_busy().await?;
        }
        Ok(())
    }
    ...
    pub async fn power_down(&mut self) -> Result<()> {
        log::debug!("Sending Powerdown command");
        self.command(Command::Powerdown).await
    }
    ...
    /// Note that this does *not* erase the flash beforehand; use `program()` for a higher-level
    /// erase-program-verify interface.
    pub async fn program_data(&mut self, address: u32, data: &[u8]) -> Result<()> {
        self.program_data_cb(address, data, |_| {}).await
    }
    ...
    /// use `program()` for a higher-level erase-program-verify interface.
    #[cfg(feature = "std")]
    pub async fn program_data_progress(&mut self, address: u32, data: &[u8]) -> Result<()> {
        let pb = ProgressBar::new(data.len() as u64).with_style(
            ProgressStyle::with_template(Self::DATA_PROGRESS_TPL)
                .unwrap()
                .progress_chars(Self::DATA_PROGRESS_CHARS),
    ...
    /// Calls `cb` with the number of bytes programmed so far after each
    /// page programming operation.
    pub async fn program_data_cb<F: Fn(usize)>(
        &mut self,
        address: u32,
        mut data: &[u8],
        cb: F,
        ...
        if first_write != page_size {
            log::trace!("Programming partial first page of {} bytes", first_write);
            self.page_program(address, &data[..first_write]).await?;
            total_bytes += first_write;
            data = &data[first_write..];
            cb(total_bytes);
        }
    ...
    pub async fn write_enable(&mut self) -> Result<()> {
        self.command(Command::WriteEnable).await
    }
    ...
    ///
    /// Returns (bank, manufacturer ID, device ID).
    pub async fn read_jedec_id(&mut self) -> Result<(u8, u8, u16)> {
        // Attempt to read assuming a single-byte manufacturer ID.
        let data = self.exchange(Command::ReadJEDECID, &[], 3).await?;
        if data[0] != 0x7F {
            Ok((0, data[0], u16::from_be_bytes([data[1], data[2]])))
        } else {
            // If the first byte is continuation, read 16 bytes, to allow
            // up to 13 continuation bytes, and then parse it to find the IDs.
            let data = self.exchange(Command::ReadJEDECID, &[], 16).await?;
            for n in 1..=13 {
                if data[n] != 0x7F {
                    return Ok((
                        n as u8,
    ...
    ///
    /// Returns (bank, manufacturer ID, device ID).
    pub async fn read_device_id(&mut self) -> Result<(u8, u8, u8)> {
        // Attempt to read assuming a single-byte manufacturer ID.
        let data = self.exchange(Command::ReadDeviceID, &[0, 0, 0], 2).await?;
        if data[0] != 0x7F {
            Ok((0, data[0], data[1]))
        } else {
            // If the first byte is continuation, read 15 bytes, to allow
            // up to 13 continuation bytes, and then parse it to find the IDs.
            let data = self.exchange(Command::ReadJEDECID, &[0, 0, 0], 15).await?;
            for n in 1..=13 {
                if data[n] != 0x7F {
                    return Ok((n as u8, data[n], data[n + 1]));
                }
    ...
    /// Read the device's 64-bit unique ID, if present.
    pub async fn read_unique_id(&mut self) -> Result<u64> {
        self.exchange(Command::ReadUniqueID, &[0, 0, 0, 0], 8)
            .await
            .map(|data| u64::from_be_bytes(data.try_into().unwrap()))
    }
    ...
    /// Read status register 1.
    pub async fn read_status1(&mut self) -> Result<StatusRegister1> {
        self.exchange(Command::ReadStatusRegister1, &[], 1)
            .await
            .map(|data| StatusRegister1(data[0]))
    }
    ...
    /// This status register is less widely supported and SFDP does
    /// not indicate whether or not it is present.
    pub async fn read_status2(&mut self) -> Result<StatusRegister2> {
        self.exchange(Command::ReadStatusRegister2, &[], 1)
            .await
            .map(|data| StatusRegister2(data[0]))
    }
    ...
    /// This status register is less widely supported and SFDP does
    /// not indicate whether or not it is present.
    pub async fn read_status3(&mut self) -> Result<StatusRegister3> {
        self.exchange(Command::ReadStatusRegister3, &[], 1)
            .await
            .map(|data| StatusRegister3(data[0]))
    }
    ...
    /// to enable writing to status register 1, that is used, otherwise the
    /// default WriteEnable of 0x06 is used.
    async fn write_status1(&mut self, status1: StatusRegister1) -> Result<()> {
        let we_opcode = if let Some(params) = self.params {
            match params.status_1_vol {
                Some(SFDPStatus1Volatility::NonVolatile06) => 0x06,
                Some(SFDPStatus1Volatility::Volatile06) => 0x06,
    ...
    pub async fn is_busy(&mut self) -> Result<bool> {
        ...
        // Otherwise and by default, poll status register 1 instead.
        self.read_status1().await.map(|status| status.get_busy())
    }
    ...
    /// This polls using `is_busy()`, which uses the flag status
    /// register if available or otherwise uses status register 1.
    pub async fn wait_while_busy(&mut self) -> Result<()> {
        while self.is_busy().await? {}
        Ok(())
    }
    ...
    /// Writes `command` and `data` to the flash memory, then returns `nbytes` of response.
    pub async fn exchange<C: Into<u8>>(
        &mut self,
        command: C,
        data: &[u8],
        nbytes: usize,
        ...
        log::trace!("SPI exchange: write {:02X?}, read {} bytes", &tx, nbytes);
        tx.extend(alloc::vec![0u8; nbytes]);
        let rx = self.access.exchange(&tx).await?;
        log::trace!("SPI exchange: read {:02X?}", &rx[1 + data.len()..]);
        Ok(rx[1 + data.len()..].to_vec())
    }
    ...
    pub async fn write<C: Into<u8>>(&mut self, command: C, data: &[u8]) -> Result<()> {
        ...
        tx.extend(data);
        log::trace!("SPI write: {:02X?}", &tx);
        self.access.write(&tx).await?;
        Ok(())
    }

    /// Convenience method for issuing a single command and not caring about the returned data
    pub async fn command<C: Into<u8>>(&mut self, command: C) -> Result<()> {
    ...
    /// Read all the bytes before `address` in memory which will be erased by `plan`.
    async fn read_erase_preamble(&mut self, address: u32, plan: &ErasePlan) -> Result<Vec<u8>> {
        let base = plan.0[0].2;
        let len = address - base;
        if len > 0 {
            log::debug!("Reading erase preamble: base={} len={}", base, len);
            self.read(base, len as usize).await
        } else {
            Ok(Vec::new())
        }
    }
    ...
    async fn read_erase_postamble(
        ...
        if len > 0 {
            log::debug!("Reading erase postamble: addr={} len={}", start, len);
            let data = self.read(start, len).await?;
            // If all the postamble is already 0xFF, there's no point reprogramming it.
            if data.iter().all(|x| *x == 0xFF) {
                Ok(Vec::new())
            } else {
    ...
    ///
    /// `cb` is called with the number of bytes erased so far.
    async fn run_erase_plan<F: Fn(usize)>(&mut self, plan: &ErasePlan, cb: F) -> Result<()> {
        let mut total_erased = 0;
        cb(total_erased);
        for (opcode, size, base, duration) in plan.0.iter() {
            log::trace!(
            ...
            let addr = self.make_address(*base);
            self.write_enable().await?;
            self.write(*opcode, &addr).await?;
            if let Some(duration) = duration {
                self.access.delay(*duration / 2).await;
            }
            self.wait_while_busy().await?;
            total_erased += size;
            cb(total_erased);
    ...
    /// to the terminal.
    #[cfg(feature = "std")]
    async fn run_erase_plan_progress(&mut self, plan: &ErasePlan) -> Result<()> {
        let erase_size = plan.total_size() as u64;
        let pb = ProgressBar::new(erase_size).with_style(
            ProgressStyle::with_template(Self::DATA_PROGRESS_TPL)
                .unwrap()
    ...
    ///
    /// Returns Err::ReadbackError on mismatch.
    async fn verify_readback(&mut self, address: u32, data: &[u8], new_data: &[u8]) -> Result<()> {
        let mismatch = data
            .iter()
            .zip(new_data)
            .enumerate()
